...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edConfirm = findViewById(R.id.textViewAppFess);
            ...
            public void onClick(View v) {
                ...
                if (username.length() == 0  || password.length() == 0 || email.length() == 0 || confirm.length() == 0 ){
                    ...
                    if(password.compareTo(confirm)==0){
                        if(isValid(password)){
                            sqLiteDatabase.register(username,email,password);
                            Toast.makeText(getApplicationContext(),"Record Inserted", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(RegisterActivity.this, LoginActivity.class));
                        }
                        else {
                           Toast.makeText(getApplicationContext(),"Password must contain at least 8 characters, having atleast 1letter, 1digit and 1symbol",Toast.LENGTH_SHORT).show();
                        }
                    }else{
